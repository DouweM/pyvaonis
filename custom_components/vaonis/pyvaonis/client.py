...
Lifecycle::
    ...
    async with VaonisClient() as scope:
...
from ._eio3 import VaonisSocketError
...
StatusCallback = Callable[[VaonisStatus], Awaitable[None] | None]


class VaonisError(Exception):
    """Base error."""


class VaonisConnectionError(VaonisError):
    """Network/transport error talking to the telescope."""


class VaonisCommandError(VaonisError):
...
class VaonisClient:
    ...
    def __init__(
        ...
        *,
        ...
        self.device_id = device_id or f"pyvaonis-{uuid.getnode():x}"
        self.name = name
        self.country_code = country_code
        self.request_timeout = request_timeout
        self.status: VaonisStatus | None = None
    ...
    # -- context management -------------------------------------------------------------
    async def __aenter__(self) -> VaonisClient:
        await self.connect()
        return self
    ...
    async def _apply_status(self, raw: dict[str, Any]) -> None:
        status = VaonisStatus.model_validate(raw)
        self.status = status
    ...
    async def connect(self, *, status_timeout: float = 15.0) -> VaonisStatus:
        ...
        except (VaonisSocketError, aiohttp.ClientError, TimeoutError, OSError) as err:
            raise VaonisConnectionError(f"socket connect failed: {err}") from err
        try:
            await asyncio.wait_for(self._first_status.wait(), timeout=status_timeout)
        except TimeoutError as err:
            raise VaonisConnectionError(
    ...
    async def _wait_for_status(
        self, predicate: Callable[[VaonisStatus], bool], *, timeout: float, desc: str
    ) -> None:
        ...
        def _check(status: VaonisStatus) -> None:
            if predicate(status):
                event.set()
        ...
        try:
            ...
            raise VaonisCommandError(f"timed out after {timeout:.0f}s waiting for {desc}") from err
        finally:
    ...
    def _require_control(self, action: str) -> None:
        if self.status is None:
            raise VaonisCommandError(f"{action}: not connected (no status yet)")
        if self.status.shutting_down:
            raise VaonisCommandError(f"{action}: telescope is shutting down")
    ...
    def _auth_header(self) -> str:
        if self.status is None:
            raise VaonisCommandError("no status yet; call connect() first")
        ...
        *,
        ...
        try:
            async with self._session.request(
                ...
                if resp.status >= 400:
                    raise VaonisCommandError(f"{endpoint} -> HTTP {resp.status}: {text}")
                try:
    ...
    async def observe_object(
        ...
        try:
            ...
            raise VaonisCommandError(str(err)) from err
    ...
    async def start_plan(
        ...
        *,
        ...
        ``items`` is a list of :class:`~pyvaonis.plan.PlanItem` (``target``/``minutes``). The
    ...
    async def adjust_framing(self, x: int, y: int, rot: float = 0.0) -> dict[str, Any]:
        ...
        if self.current_observation() is None:
            raise VaonisCommandError("adjust_framing: no observation in progress")
    ...
    async def fetch_image(self, image: LiveImage | str) -> bytes:
        ...
        try:
            async with self._session.get(url, timeout=timeout) as resp:
                if resp.status >= 400:
                    raise VaonisCommandError(f"image {url} -> HTTP {resp.status}")
                return await resp.read()
        except aiohttp.ClientError as err:
            raise VaonisConnectionError(f"image fetch failed: {err}") from err
    ...
    async def export_url(self, capture_id: str, fmt: str = "tiff") -> str:
        ...
        if fmt == "tiff":
            ...
            raise VaonisCommandError(f"unknown export format: {fmt!r} (use 'tiff' or 'jxl')")
        url = (resp.get("result") or {}).get("url")
        if not url:
            raise VaonisCommandError(f"export of {capture_id} returned no url: {resp}")
    ...
    async def latest_capture(self) -> FtpEntry | None:
        """The last frame (an :class:`FtpEntry`, with its modify time) in the newest finished run.
    ...
    async def switch_frequency(self, band: str, *, force: bool = False) -> dict[str, Any]:
        ...
        if band not in (const.BAND_2_4_GHZ, const.BAND_5_GHZ):
            raise VaonisCommandError(f"invalid band {band!r} (use BAND_2_4_GHZ or BAND_5_GHZ)")
